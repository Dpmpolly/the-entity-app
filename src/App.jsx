...
const OnboardingWizard = ({ onComplete }) => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950 p-6">
        <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl text-center">
            <h1 className="text-3xl font-black text-white italic mb-6"><Skull className="inline text-purple-500 mb-1"/> THE ENTITY</h1>
            
            {step === 1 && <>
                <h2 className="text-xl font-bold text-white mb-4">Duration</h2>
                {[30, 90, 365].map(d => (
                    <button key={d} onClick={() => setDuration(d)} className={`w-full p-4 mb-2 rounded border text-left flex justify-between ${duration === d ? 'border-purple-500 bg-purple-900/20' : 'border-slate-700'}`}>
                        <span className="font-bold text-white">{d} Days</span>
                        {duration === d && <CheckCircle2 className="text-purple-500"/>}
                    </button>
                ))}
                <button onClick={() => setStep(2)} className="w-full bg-purple-600 text-white font-bold py-3 rounded mt-4">Next</button>
            </>}

            {step === 2 && <>
                <h2 className="text-xl font-bold text-white mb-4">Difficulty</h2>
                {Object.values(DIFFICULTIES).map(d => (
                    <button key={d.id} onClick={() => setDifficulty(d.id)} className={`w-full p-4 mb-2 rounded border text-left flex justify-between ${difficulty === d.id ? 'border-purple-500 bg-purple-900/20' : 'border-slate-700'}`}>
                        <span className={`font-bold ${d.color}`}>{d.label}</span>
                    </button>
                ))}
                <div className="flex gap-2 mt-4">
                    <button onClick={() => setStep(1)} className="px-4 text-slate-400">Back</button>
                    <button onClick={() => setStep(3)} className="flex-1 bg-purple-600 text-white font-bold py-3 rounded">Next</button>
                </div>
            </>}

            {step === 3 && <>
                <h2 className="text-xl font-bold text-white mb-4">Codename</h2>
                <input className="w-full bg-slate-800 border border-slate-700 rounded p-3 text-white text-center mb-4 uppercase" placeholder="AGENT NAME" value={codename} onChange={e => setCodename(e.target.value)} />
                <div className="flex gap-2 mt-4">
                    <button onClick={() => setStep(2)} className="px-4 text-slate-400">Back</button>
                    <button disabled={!codename} onClick={() => setStep(4)} className="flex-1 bg-purple-600 text-white font-bold py-3 rounded disabled:opacity-50">Next</button>
                </div>
            </>}
            ...
            {step === 4 && <>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                    {Object.values(AVATARS).map(a => (
                        <button key={a.id} onClick={() => setAvatarId(a.id)} className={`p-4 border rounded flex flex-col items-center ${avatarId === a.id ? 'border-purple-500 bg-purple-900/20' : 'border-slate-700'}`}>
                            <a.icon className={`text-white mb-1 ${a.bg} p-1 rounded-full`} size={24}/>
                            <span className="text-xs text-white font-bold">{a.name}</span>
                        </button>
                    ))}
                </div>
                <button onClick={() => onComplete({ duration, avatarId, difficulty, username: codename })} className="w-full bg-emerald-600 text-white font-bold py-3 rounded">INITIALIZE</button>
            </>}
        </div>
      </div>
    );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 p-6">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-sm p-6">
            <h3 className="font-bold text-white mb-4">Manual Log</h3>
            <input type="number" className="w-full bg-slate-800 border border-slate-700 rounded p-3 mb-2 text-white" placeholder="Distance (km)" value={km} onChange={e => setKm(e.target.value)} />
            <input type="text" className="w-full bg-slate-800 border border-slate-700 rounded p-3 mb-4 text-white" placeholder="Notes" value={notes} onChange={e => setNotes(e.target.value)} />
            {activeQuest?.status === 'active' && (
                <div onClick={() => setIsQuestRun(!isQuestRun)} className={`p-3 border rounded mb-4 flex items-center gap-2 cursor-pointer ${isQuestRun ? 'border-amber-500 bg-amber-900/20' : 'border-slate-700'}`}>
                    <div className={`w-4 h-4 border rounded ${isQuestRun ? 'bg-amber-500' : ''}`}></div>
                    <span className="text-sm text-slate-300">Apply to Quest</span>
                </div>
            )}
            <div className="flex gap-2">
                <button onClick={onClose} className="flex-1 p-3 text-slate-400">Cancel</button>
                <button onClick={() => { if(km) { onSave(km, notes, isQuestRun); onClose(); } }} className="flex-1 bg-emerald-600 text-white rounded font-bold">Save</button>
            </div>
        </div>
      </div>
    );
...
export default function TheEntity() {
  ...
  const handleBuyBoost = async () => {
      ...
      const boost = parseFloat((gameState.totalKmRun * 0.01).toFixed(2)); // simplified for stability
      await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'game_data', 'main_save'), {
